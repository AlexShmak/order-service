...
func NewRouter(storage *storage.PostgresStorage, logger *slog.Logger, jwtService *auth.JWTService, cfg *config.Config) *gin.Engine {
	router := gin.Default()
	...
	handler := handlers.NewHandler(storage, logger, jwtService, cfg)
